 ...
 * Lida com referências/consultas anuláveis.
 * 
 *
 * IMPORTANTE! VOCÊ DEVE MEMOIZAR o memoizedTargetRefOrQuery de entrada ou COISAS RUINS ACONTECERÃO
 * use useMemo para memoizá-lo conforme a orientação do React. Certifique-se também de que suas dependências são referências estáveis
 *  
 * @template T Tipo opcional para dados de documentos. O padrão é any.
 * @param {CollectionReference<DocumentData> | Query<DocumentData> | null | undefined} targetRefOrQuery -
 * A CollectionReference ou Query do Firestore. Aguarda se for nulo/indefinido.
 * @returns {UseCollectionResult<T>} Objeto com dados, isLoading, error.
 */
export function useCollection<T = any>(
  ...
  useEffect(() => {
    ...
    // Usa diretamente memoizedTargetRefOrQuery, pois presume-se que seja a consulta final
    const unsubscribe = onSnapshot(
      ...
      (error: FirestoreError) => {
        // Esta lógica extrai o caminho de uma referência ou de uma consulta
        const path: string =
        ...
        // aciona a propagação global de erros
