  ...
  const handleCreate = async () => {
    if (!title || !subject || !studentId) {
      toast({
        ...
        description: "Preencha o título, o aluno e a matéria.",
      });
      return;
    }
    ...
    for (let i = 0; i < draftQuestions.length; i++) {
      ...
      if (
        ...
        toast({
          ...
          description: `Preencha todos os campos da Questão ${i + 1}.`,
        });
        return;
      }
    }
    ...
    if (!result.success) {
      toast({
        ...
        title: "Erro ao criar",
        description:
          result.error || "Falha ao criar simulado no banco de dados.",
      });
    ...
    toast({
      ...
      description: `A prova com ${questions.length} questões foi salva e enviada.`,
    });
    resetForm();
    setIsCreating(false);
    setIsSubmitting(false);
          ...
          <CardHeader className="bg-amber-50 border-b border-amber-100 pb-5 flex flex-row items-start justify-between">
            <div>
              ...
              <CardDescription className="text-amber-800 font-medium">
                Configure a prova e adicione quantas questões quiser no
                construtor abaixo.
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent className="pt-6 bg-white space-y-6">
            ...
            <div className="grid gap-4 bg-slate-50 p-6 rounded-2xl border border-slate-200">
              <h3 className="font-black text-slate-800 uppercase tracking-wider text-sm flex items-center gap-2">
                <Settings className="h-4 w-4" /> 1. Configurações da Prova
              </h3>
              <div className="grid md:grid-cols-2 gap-4 mt-2">
                <div className="grid gap-2">
                  ...
                  <Input
                    ...
                    placeholder="Ex: Simulado ENEM 2026 - Matemática"
                    value={title}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    ...
                    <Input
                      className="bg-white h-12 text-center text-lg font-bold"
                      type="number"
                      min={1}
                      value={maxAttempts}
            ...
            <div className="grid gap-4">
              <div className="flex items-center justify-between">
                <h3 className="font-black text-slate-800 uppercase tracking-wider text-sm flex items-center gap-2">
                  <ListPlus className="h-5 w-5" /> 2. Construtor de Questões (
              ...
              <div className="space-y-6">
                {draftQuestions.map((q, qIndex) => (
                  <div
                    key={qIndex}
                    className="relative p-6 bg-white border-2 border-slate-200 rounded-3xl shadow-sm hover:border-brand-yellow/50 transition-colors group"
                  >
                    <div className="flex items-center justify-between mb-4">
                      <Badge className="bg-slate-800 text-white font-black text-sm px-3 py-1">
                        Questão {qIndex + 1}
                      </Badge>
                    ...
                    <div className="grid gap-4">
                      <div className="grid gap-2">
                        ...
                        <Textarea
                          className="bg-slate-50 text-base font-medium border-slate-300 focus:border-brand-yellow"
                          rows={3}
                      ...
                      <div className="grid gap-3 mt-2">
                        ...
                        {["A", "B", "C", "D"].map((letter, optIndex) => (
                          <div key={letter} className="flex items-center gap-3">
                            <Badge className="w-8 h-8 flex items-center justify-center bg-slate-200 text-slate-700 text-lg shadow-none">
                              {letter}
                            </Badge>
                            <Input
                      ...
                      <div className="grid gap-2 mt-2 p-4 bg-green-50/50 rounded-2xl border border-green-100">
                        <Label className="font-black text-green-800 uppercase tracking-wider text-xs">
                          Resposta Correta (Gabarito da Q.{qIndex + 1})
                        </Label>
                        <Select
              ...
              <Button
                variant="outline"
                className="w-full h-14 border-2 border-dashed border-slate-300 text-slate-500 hover:border-brand-yellow hover:text-brand-yellow hover:bg-amber-50 font-black uppercase tracking-wider rounded-2xl transition-all"
                onClick={addQuestion}
              >
            ...
            <div className="pt-6 border-t mt-4">
              <Button
                ...
                className="w-full h-16 rounded-2xl bg-brand-yellow font-black text-slate-900 text-xl shadow-lg hover:-translate-y-1 hover:bg-amber-400 transition-all"
                onClick={handleCreate}
              >
                {isSubmitting
                  ? "Salvando Prova..."
                  : `Finalizar e Enviar Prova (${draftQuestions.length} Questões)`}
              </Button>
            </div>
          </CardContent>
        </Card>
