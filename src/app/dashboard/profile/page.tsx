              ...
              <Card className="rounded-3xl border-slate-200 shadow-sm overflow-hidden transition-colors hover:border-[#f5b000]">
                <CollapsibleTrigger asChild>
                  <CardHeader className="bg-slate-50 border-b pb-3 pt-4 rounded-t-3xl cursor-pointer select-none hover:bg-slate-100 transition-colors">
                    <CardTitle className="flex items-center justify-between text-xl text-slate-800">
                      <span className="flex items-center gap-2">
                        <Link className="h-6 w-6 text-brand-yellow" />
                        Integrações
                      </span>
                      <ChevronDown className={`h-5 w-5 text-slate-400 transition-transform duration-200 ${openSection === 'integrations' ? 'rotate-180' : ''}`} />
                    </CardTitle>
                    <CardDescription className="text-base font-medium">
                      Conecte serviços externos para sincronizar sua agenda.
                    </CardDescription>
                  </CardHeader>
                </CollapsibleTrigger>
                <CollapsibleContent>
                  <CardContent className="pt-6 space-y-4">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500">
                            {(currentUser as any)?.googleCalendarConnected 
                              ? `Conectado como ${(currentUser as any)?.googleCalendarEmail}` 
                              : 'Sincronize sua agenda do Google'}
                          </p>
                        </div>
                      </div>
                      <Button
                        variant={(currentUser as any)?.googleCalendarConnected ? "outline" : "default"}
                        className={(currentUser as any)?.googleCalendarConnected 
                          ? "border-red-200 text-red-600 hover:bg-red-50" 
                          : "bg-blue-600 hover:bg-blue-700 text-white"}
                        onClick={() => {
                          if ((currentUser as any)?.googleCalendarConnected) {
                            // Desconectar
                            toast({ title: "Em desenvolvimento", description: "Funcionalidade de desconexão em breve." });
                          } else {
                            // Conectar
                            toast({ title: "Em desenvolvimento", description: "Configure as credenciais do Google OAuth primeiro." });
                          }
                        }}
                      >
                        {(currentUser as any)?.googleCalendarConnected ? (
                    ...
                    <p className="text-sm text-slate-500 bg-amber-50 p-3 rounded-lg border border-amber-200">
                      ...
                      Para ativar esta funcionalidade, configure as credenciais OAuth do Google nas variáveis de ambiente.
                    </p>
                  </CardContent>
                </CollapsibleContent>
              </Card>
