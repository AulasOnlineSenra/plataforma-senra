...
export default function MinhasAulasPage() {
  ...
  const [confirmingLesson, setConfirmingLesson] = useState<LessonItem | null>(
    null,
  );
  ...
  const loadLessons = async (currentUserId: string, currentRole: string) => {
    ...
    if (response.success && response.data) {
      const now = new Date();
      setLessons(
        (response.data as LessonItem[]).filter((l) => new Date(l.date) >= now),
      );
    } else {
      toast({
        variant: "destructive",
  ...
  const handleConfirmLesson = async () => {
    ...
    setSaving(true);
    const result = await confirmLesson(
      confirmingLesson.id,
      userId,
      meetingLink,
    );
    ...
    if (result.success) {
      toast({
        ...
        description: "O link da sala foi salvo com sucesso.",
      });
  ...
  return (
    <div className="flex flex-1 flex-col gap-6">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge
                      variant="outline"
                      className={
                        statusStyles[lesson.status] || statusStyles.PENDING
                      }
                    >
                      {lesson.status}
                    </Badge>
      ...
      <Dialog
        open={!!confirmingLesson}
        onOpenChange={() => setConfirmingLesson(null)}
      >
        <DialogContent className="rounded-3xl border-slate-200">
          <DialogHeader>
            <DialogTitle className="text-slate-900">Confirmar Aula</DialogTitle>
            <DialogDescription>
              Cole o link da sala do Google Meet para liberar acesso ao aluno.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setConfirmingLesson(null)}
