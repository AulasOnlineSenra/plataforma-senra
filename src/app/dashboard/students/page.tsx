...
import { CalendarCheck, MessageSquare, MoreHorizontal, Plus, Trash2, Wallet, Users } from 'lucide-react';
...
function StudentList({
  ...
  return (
    <Card id={id} className="rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      <CardHeader className="border-b border-slate-100 pb-4 pt-6 px-6 md:px-8 bg-slate-50/30">
        <CardTitle className="text-xl text-slate-900 font-bold">{title}</CardTitle>
      </CardHeader>
      <CardContent className="p-0">
        <Table className="divide-y divide-slate-100">
          <TableHeader>
            <TableRow className="border-slate-100 bg-slate-50/50 hover:bg-slate-50/50">
              <TableHead className="w-16 px-6 font-semibold text-slate-500">Foto</TableHead>
              ...
              <TableHead className="font-semibold text-slate-500">Último Acesso</TableHead>
              <TableHead className="text-center font-semibold text-slate-500">Aulas Agendadas</TableHead>
              <TableHead className="text-right px-6 font-semibold text-slate-500">Ações</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {students.length > 0 ? (
              students.map((student) => (
                <TableRow key={student.id} className="border-slate-100 hover:bg-slate-50/80 transition-colors">
                  ...
                  <TableCell className="text-slate-500 font-medium py-4">{student.email}</TableCell>
                  <TableCell className="text-slate-400 text-sm py-4">{formatLastAccess(student)}</TableCell>
                  ...
                  <TableCell className="text-right px-6 py-4">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" aria-label={`Ações para ${student.name}`} className="h-10 w-10 rounded-full text-slate-400 hover:bg-slate-100 hover:text-slate-700">
                          <MoreHorizontal className="h-5 w-5" />
                        </Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent align="end" className="w-56 rounded-2xl border border-slate-100 bg-white shadow-lg p-2 gap-1 flex flex-col">
                        <DropdownMenuItem onSelect={() => onAddCredits(student)} className="cursor-pointer rounded-xl py-2.5 font-medium text-slate-700 focus:bg-slate-50 focus:text-slate-900">
                          <Wallet className="mr-2 h-4 w-4 text-emerald-500" />
                          Adicionar créditos
                        </DropdownMenuItem>
                        <DropdownMenuItem onSelect={() => router.push(`/dashboard/chat?contactId=${student.id}`)} className="cursor-pointer rounded-xl py-2.5 font-medium text-slate-700 focus:bg-slate-50 focus:text-slate-900">
...
export default function AdminStudentsPage() {
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      toast({ title: 'Sucesso!', description: 'Aluno cadastrado com sucesso.', className: 'bg-emerald-600 text-white border-none' });
  ...
  const handleAddCreditsSubmit = async (e: React.FormEvent) => {
    ...
    if (!Number.isInteger(normalizedCredits) || normalizedCredits <= 0) {
      toast({ variant: 'destructive', title: 'Erro', description: 'Informe uma quantidade de créditos valida.' });
      return;
    }
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Erro', description: 'Falha inesperada ao registrar transação.' });
    } finally {
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 md:gap-8">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Gestão de Alunos</h1>
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="sm:max-w-md rounded-3xl border border-slate-100 bg-white shadow-xl p-8">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-slate-900">Novo Aluno</DialogTitle>
            <DialogDescription className="text-slate-500">Crie o acesso de um novo aluno manualmente.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreateSubmit} className="grid gap-5 py-4">
            <div className="grid gap-2">
              <Label htmlFor="create-name" className="font-bold text-slate-700">Nome Completo</Label>
              <Input
                id="create-name"
                value={formData.name}
            ...
            <div className="grid gap-2">
              <Label htmlFor="create-email" className="font-bold text-slate-700">E-mail de Acesso</Label>
              <Input
                id="create-email"
                type="email"
            ...
            <div className="grid gap-2">
              <Label htmlFor="create-password" className="font-bold text-slate-700">Senha Temporária</Label>
              <Input
                id="create-password"
                type="password"
            ...
            <DialogFooter className="mt-6 gap-3 sm:gap-0">
              <Button type="button" variant="ghost" onClick={() => setIsCreateOpen(false)} className="rounded-xl font-bold text-slate-500 hover:text-slate-700">
                Cancelar
              </Button>
      ...
      >
        <DialogContent className="sm:max-w-md rounded-3xl border border-slate-100 bg-white shadow-xl p-8">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-slate-900">Registrar Venda</DialogTitle>
            <DialogDescription className="text-slate-500">
              Libere créditos para <span className="font-bold text-slate-700">{studentToCredit?.name}</span> e gere o recibo.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleAddCreditsSubmit} className="grid gap-5 py-4">
            <div className="grid gap-2">
              <Label htmlFor="credit-plan-name" className="font-bold text-slate-700">Nome do Pacote/Plano</Label>
              <Input
                id="credit-plan-name"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                <Label htmlFor="credit-amount-paid" className="font-bold text-slate-700">Valor Pago (R$)</Label>
                <Input
              ...
              <div className="grid gap-2">
                <Label htmlFor="credit-quantity" className="font-bold text-slate-700">Qtd. de Aulas</Label>
                <Input
                  id="credit-quantity"
                  type="number"
                  min={1}
            ...
            <div className="grid gap-2">
              <Label htmlFor="credit-payment-method" className="font-bold text-slate-700">Método de Pagamento</Label>
              <Select
                value={creditForm.paymentMethod}
                onValueChange={(value) => setCreditForm((prev) => ({ ...prev, paymentMethod: value }))}
              >
                <SelectTrigger id="credit-payment-method" className="h-12 rounded-xl border-slate-200 focus:ring-brand-yellow">
                  <SelectValue placeholder="Selecione o método" />
                </SelectTrigger>
                <SelectContent className="rounded-xl">
                  <SelectItem value="PIX" className="cursor-pointer">PIX</SelectItem>
                  <SelectItem value="Cartao de Credito" className="cursor-pointer">Cartão de Crédito</SelectItem>
            ...
            <DialogFooter className="mt-6 gap-3 sm:gap-0">
              <Button type="button" variant="ghost" onClick={() => setIsAddCreditsOpen(false)} disabled={isAddingCredits} className="rounded-xl font-bold text-slate-500 hover:text-slate-700">
                Cancelar
              </Button>
              <Button type="submit" disabled={isAddingCredits} className="h-11 rounded-xl bg-brand-yellow px-6 font-bold text-slate-900 shadow-sm hover:bg-brand-yellow/90">
      ...
      <AlertDialog open={!!studentToDelete} onOpenChange={(open) => !open && setStudentToDelete(null)}>
        <AlertDialogContent className="rounded-3xl border-slate-100 p-8 shadow-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-2xl font-bold text-slate-900">Excluir aluno?</AlertDialogTitle>
            <AlertDialogDescription className="text-base text-slate-500 mt-2">
              Esta ação remove permanentemente o perfil de <span className="font-bold text-slate-900">{studentToDelete?.name}</span>. Todos os dados vinculados podem ser perdidos.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-6">
            <AlertDialogCancel className="rounded-xl border-slate-200 font-bold text-slate-600 hover:bg-slate-50 h-11 px-6">Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteStudent} className="rounded-xl bg-red-600 font-bold text-white hover:bg-red-700 h-11 px-8">
